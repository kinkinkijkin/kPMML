...
namespace kinkaudio
{
	public class FM
	{
		...
		{
			if (truncMod == 0 && truncCar == 0) return Convert.ToSingle(Math.Sin(time / carrierPeriod + inOp) / amplitude);
			else
			{
				if (truncMod == 1) inOp = Math.Max(0, inOp);
				else if (truncMod == 2) inOp = Math.Min(0, inOp);
				if (truncCar == 1) return Convert.ToSingle(Math.Max(0, Math.Sin(time / carrierPeriod + inOp)) / amplitude);
				else if (truncCar == 2) return Convert.ToSingle(Math.Min(0, Math.Sin(time / carrierPeriod + inOp)) / amplitude);
				else return Convert.ToSingle(Math.Sin(time / carrierPeriod + inOp) / amplitude);
			}
		}
